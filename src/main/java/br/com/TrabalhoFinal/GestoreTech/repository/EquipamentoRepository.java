...
public interface EquipamentoRepository extends JpaRepository<EquipamentoEntity, Integer> {
	...
	@Query("SELECT e FROM EquipamentoEntity e WHERE e.cliente.id = :clienteId " +
	       "AND (:numeroSerie IS NULL OR :numeroSerie = '' OR LOWER(e.numeroSerie) LIKE LOWER(CONCAT('%', :numeroSerie, '%'))) " +
